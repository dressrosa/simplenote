 ...
 */
package com.xiaoyu.simplenote.modules.biz.article.service;
...
import com.xiaoyu.simplenote.common.utils.JedisUtils;
...
import com.xiaoyu.simplenote.modules.biz.article.dao.ArticleAttrDao;
import com.xiaoyu.simplenote.modules.biz.article.dao.ArticleCollectDao;
import com.xiaoyu.simplenote.modules.biz.article.dao.ArticleColumnDao;
import com.xiaoyu.simplenote.modules.biz.article.dao.ArticleCommentDao;
import com.xiaoyu.simplenote.modules.biz.article.dao.ArticleDao;
import com.xiaoyu.simplenote.modules.biz.article.dao.ArticleLikeDao;
...
import com.xiaoyu.simplenote.modules.biz.article.entity.ArticleComment;
import com.xiaoyu.simplenote.modules.biz.article.entity.ArticleLike;
import com.xiaoyu.simplenote.modules.biz.article.entity.CommentLike;
import com.xiaoyu.simplenote.modules.biz.article.service.api.IArticleService;
import com.xiaoyu.simplenote.modules.biz.article.vo.ArticleCommentVo;
...
import com.xiaoyu.simplenote.modules.common.MailBuilder;
import com.xiaoyu.simplenote.modules.constant.BizAction;
import com.xiaoyu.simplenote.modules.constant.BizType;
import com.xiaoyu.simplenote.modules.constant.Flag;
import com.xiaoyu.simplenote.modules.constant.MsgType;
import com.xiaoyu.simplenote.modules.constant.NumCountType;
...
@Primary
@BeaconExporter(interfaceName = "com.xiaoyu.simplenote.modules.biz.article.service.api.IArticleService", group = "dev")
public class ArticleServiceImpl implements IArticleService {
    ...
    public ResponseMapper hotList(TraceRequest request) {
        ...
        List<Map<String, Object>> total = this.handleArticleVoList(list, request);
    ...
    private List<Map<String, Object>> handleArticleVoList(final List<ArticleVo> list, TraceRequest request) {
        int size = list.size();
        if (size == 0) {
        ...
        if (request.isLogin()) {
            List<String> articleIds = new ArrayList<>(size);
            // 设置查询条件
            for (ArticleVo a : list) {
                articleIds.add(a.getUuid());
            }
            likeList = this.likeDao.findByList(new CommonQuery()
                    .setUserId(request.getUser().getUuid())
                    .setBizIds(articleIds));
            collectList = this.collectDao.findByList(new CommonQuery()
                    .setUserId(request.getUser().getUuid())
                    .setBizIds(articleIds));
        }
        List<String> userIdList = list.stream().map(ArticleVo::getUserId).collect(Collectors.toList());
        // 批量查询
        Map<String, UserVo> voMap = new HashMap<>(16);
        if (!userIdList.isEmpty()) {
            List<UserVo> userVoList = this.userService.findVoByUuid(userIdList);
            voMap = userVoList.stream().collect(Collectors.toMap(UserVo::getUserId, cvo -> cvo));
        }
        // 封装数据
    ...
    public void doInsertArticle(final Article t, final ArticleContentVo co, final ArticleAttr attr,
            final String userId) {
        try {
            this.articleDao.insert(t);
            this.articleAttrDao.insert(attr);
            this.articleDao.insertContent(co);
            // 增加发文数
        ...
        // try {
        // BaseProducer.produce(Type.QUEUE, MqContant.EMAIL,
        // JSON.toJSONString(builder));
        // } catch (Exception e) {
        // e.printStackTrace();
        // }
    ...
    public ResponseMapper editArticle(TraceRequest request, String title, String content, String userId,
        ...
        temp.setBrief(content.length() > 60 ? content.substring(0, 59) : content)
                .setTitle(title)
                .setUuid(articleId);
        ArticleContentVo co = new ArticleContentVo();
    ...
    @Transactional(readOnly = false, rollbackFor = RuntimeException.class)
    public void doEdit(Article temp, ArticleContentVo co) {
        this.articleDao.update(temp);
        this.articleDao.updateContent(co);
    }
    ...
    private boolean addArticleLikeNum(ArticleLike a) {
        ...
        if (like != null) {
            ...
            if (this.likeDao.insert(a.setStatus(1).setNum(1)) > 0) {
                isSendMsg = true;
            }
        }
    ...
    @Override
    public ResponseMapper getContentByArticleId(TraceRequest request, String articleId) {
        ArticleContentVo vo = this.articleDao.getContentByArticleId(articleId);
        return ResponseMapper.createMapper().data(vo);
    }
