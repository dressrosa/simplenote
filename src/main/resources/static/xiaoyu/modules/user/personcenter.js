					...
					.each(
							...
							function(index, ar) {
								arHtml += '<li class="list-group-item"   id="'
										+ ar.articleId + '">';
								arHtml += '<label style="color:#cd5c5c;">'
										+ ar.title + '</label>';
								...
								if (userInfo != null
									...
									arHtml += '<div class="group_item_edit_part">'
											+ '<label  class="edit_part_label edit_label">编辑</label>'
											+ '<label  class="edit_part_label remove_label">删除</label>'
											+ '</div>';
								}
				...
				function() {
					...
					$(".tab_ul")
							.on(
									...
									function() {
										var $selected = $(this);
										$.each($selected.siblings(), function(
												i, v) {
