...
$(function() {
	...
	var url = window.location.hostname === 'blueimp.github.io' ? '//jquery-file-upload.appspot.com/'
			: '/api/v1/upload/avatar', uploadButton = $('<button />').addClass(
			'btn').addClass('sub').prop('disabled', true).text('上传中').on(
			'click', function() {
			...
			.on(
					...
					function(e, data) {
						...
						if (index + 1 === data.files.length) {
							data.context.find('button').text('上传图片').prop(
				...
				$('#progress .progress-bar').css('width', progress + '%');
					...
					function(e, data) {
						...
						if (r.code == '0') {
							...
							$.session.set("user", JSON.stringify(userInfo),
									true);
						} else {
						 ...
						 * file.url;
						 * 
						 * s = s.replace("avatar", "avatar/thumbnail");
